...
- v2.3.0 (tag pushed): legal search + repository. Backend: search filters (date/category/court) + full-text body/content + relevance + hearings kind; acts/books list filters; shelf metadata (doc_number/act_date/published_at/external_id/synced_at/sync_status); shelf_book_versions + rollback; related docs; sync-now endpoint (allowlist, status stamps). Frontend: modal filter UI + hearing tab, admin meta fields + versions/rollback + sync button. Tests 17/17 (versions 0→2, rollback restores, cleanup delete). tsc clean, build 5.52s, backup 8192 + restore-test green. Next: flipbook/DOCX (v2.3.1).

## v2.3.0 BEFORE - legal platform audit (from SUD_TJ_AUDIT.md + live code)
- Audit reqs: dead sud.tj search (?q=sud empty, P0 — fixed by our /api/search), docs catalog thin (P2), TJ/RU/EN search (P0 — titles covered, body NOT), no invented data rule applies.
- Gaps found: /api/search titles-only LIKE, no date/category/court/status filters; /api/judicial_acts + /api/shelf-books zero filters; shelf_books no versions/history/related, metadata lacks doc_number/act_date/published_at/external refs; no ADLIA sync mechanism (only manual import); GlobalSearchModal no filter UI; flipbook/DOCX deferred to v2.3.1.
- Plan v2.3.0: search filters (date_from/to, category, court, full-text body+content, relevance order) + acts/books list filters + shelf metadata (doc_number/act_date/published_at/external_id/synced_at/sync_status) + shelf_book_versions (snapshot-on-write, list, rollback) + related docs in detail response + sync-now endpoint (re-import by source_url, allowlist) + modal filter UI. No fixtures; migration additive.
...
## v2.9.4 FINALIZE GLOBAL LIQUID GLASS 2026-09-24 → v2.9.4
- BEFORE: белые горизонтальные линии `border-t theme-border/30` между секциями выглядели как разрывы; кнопки `btn-primary solid gold` `btn-secondary transparent` не glass, нижние карточки терялись на фоне (контрастный фон здания vs 0.14 glass), фон без overlay слишком контрастный, overlay фиксирован, настройки blur/saturation не управлялись, нет единого управления.
- AFTER: линии удалены (13 секций), все кнопки/карточки `Global Liquid Glass` (`glass_background rgba 0.14/0.19/0.23 border 0.22/0.34 blur 24 radius 24 shadow 0.12 highlight 0.42` с `backdrop-filter` fallback) — hero, information, news, document, court, regional, service, footer, tabs, filters, search, dropdowns, modals, login, admin, mobile — без opaque white/dark, без случайных border. Bottom cards `glass-strong 0.18` + highlight/shadow — видимы, текст deep navy контраст. Background `Image` → `Atmospheric White Overlay` (`opacity 0.12 blur 0 sat 100 bright 100 contrast 100` configurable, слабый, здание/небо видимы) → `Glass UI` (`glass blur 24` отдельно). Admin `Background & Glass` — Image/Position/Size, Atmosphere 5, Glass 7 — Live Preview сразу Desktop/Tablet/Mobile, presets Premium/Ultra/Clear/Soft Apply/Preview/Save/Reset, global sync Public/Admin/Login/Mobile/Footer, cross-browser `@supports -webkit fallback` без hacks, visual QA 13 пунктов PASS.
- Verify: tsc 0, build 7.42s, нет белых линий, все glass, нижние карточки видны, фон не молочный, здание видимо, glass глубина, desktop/mobile, Chrome/Edge/Firefox/Safari/iOS/Android PASS, backup verified.
...
- Verify: `http://localhost:8787/admin` теперь светлый как `/`, tsc 0, build 7.00s, background/cards/buttons/inputs/borders/radius/transparency/typography/gold едины, mobile/tablet/desktop, cross-browser @supports.

## v2.9.0 SITE_CMS_AND_LIVE_VISUAL_EDITOR 2026-09-24 → v2.9.0
- BEFORE: useful sites был static grid/snap carousel, не marquee; CMS только news/courts/acts/books/media/appeals без единого управления главной, судебной сетью, полезными сайтами, контактами, footer/header/баннерами/слайдерами/статистикой/документами/FAQ; нет visual builder, live preview, drag&drop, design tokens, section settings, draft/publish/version/rollback/roles/responsive preview.
- AFTER: marquee восстановлен — ticker дублированный track бесшовный 36s left autoplay pauseOnHover/pauseOnFocus, speed/direction/logo_size/gap/order из `site_marquee_config`+`useful_sites` (7 записей) single source, admin `UsefulSitesManager` CRUD+reorder+duplicate. CMS: `useful_sites`, `site_sections` 8, `site_design_settings` glass tokens, `site_versions` — API draft/publish/rollback, frontend published. Visual Builder `SiteBuilder` three-panel 12 Components | Live Preview (device 1920/1440/1024/768/390) | Properties (content/layout/visibility/spacing/typography/background/glass/animation/links/responsive) + DndKit не ломает layout, design presets premium/ultra/light/dark меняют Global Tokens, section visible/order/background/padding/height/glass, footer/nav/contacts/marquee управляются, draft не меняет production до Publish, version history/rollback/лог, защита hero/footer, permissions SUPER_ADMIN/ADMIN/EDITOR/MODERATOR, responsive preview, интеграция с public site.
- Verify: tsc 0, build 9.64s, marquee 7 published seamless, admin useful 200, site-sections 8, design draft/publish, DndKit, live preview immediate, backup verified.

## v2.8.1 FOOTER VISUAL INTEGRATION 2026-09-24 → v2.8.1
- BEFORE: footer `bg #050f1e` solid black + `opacity 0.07` dark gradient + circuit `opacity 0.04` gold, все карточки темные `text-slate-300/white` на темном, map `bg 5,15,30,0.35` dark, contacts `text-white`, bottom bar `rgba 5,15,30,0.65` черная полоса — визуально черная низа, не соответствует светлой верхней Liquid Glass.
- AFTER: footer `bg rgba 255,255,255,0.08 border white/20 backdrop-blur` светлый translucent, gradient светлый `0.04` white/20 линии, все карточки `Global GlassSurface 0.14 blur 24 border 0.30 radius 24-30` light `text-slate-800/700/600` deep navy `muted navy/gray` gold accent, map `bg 255,255,255,0.14`, contacts сл-900, bottom bar `rgba 255,255,255,0.14 border 0.28` deep navy text — единая физическая стеклянная поверхность header/content/footer.
- Verify: tsc 0, build 6.62s, footer не черный, нет opaque dark cards, glass единый, фон просвечивает, текст читаемый gold единый, responsive/mobile/desktop, cross-browser @supports fallback, no content loss.

## v2.8.0 COURT NETWORK 2026-09-24 → v2.8.0
- BEFORE: судебная сеть с белыми `bg-slate-100/80` regional, разными стилями верх (cyan/emerald) vs низ, яркие `colorHex` заливки/тени, пустота `py-14/lg:py-28` + `h-152 trunk` + `gap-4/5`, декор линия `6 3` rainbow + 4 glow точки, много inner `border-[#dfbe7e]/30` рамок, поиск `bg-theme-bg/70 w-80 text-xs` незаметный, списки все суды сразу (до 18 карточек в регионе), нет compact статистики.
- AFTER: Global GlassSurface unified (все карточки `glass glass-card 24px amber active` только accent dot/line), регионы одинаковые, цвет только `1.5px` dot + `2px` line top, пустота `py-10/lg:py-16 mb-4/2 gap-3`, декор линия заменена на `h-px white/10 + 4 dots + gold center`, inner рамки `white/10` subtle, иконки `MapPin 11` unified, hierarchy improved, поиск `h-11 glass white/20 gold icon` placeholder i18n live update без перезагрузки, фильтры compact glass, списки `visibleCourts 5` `Show all` expands, mobile 1 col `min-h 44` touch, статистика compact `glass 10px mono`, background темнее `050f1e` просвечивает.
- Verify: tsc 0, build 6.43s, no backend/API/route changes, 77 судов single source `REGIONAL_CLUSTERS`, responsive 4/2×2/1 без overflow, glass сохраняется, cross-browser `@supports`, a11y `aria-pressed` `aria-label` `focus ring`, perf `transform/opacity` no blur animation, no nested heavy blur.

## v2.7.0 REAL SITE FOOTER 2026-09-24 → v2.7.0
- BEFORE: Layout footer inline 278 строк: president message + 3 IA колонки + ticker полезн. сайтов (марафон) + bottom copyright/IT. Старый сплошной голубой стиль заменен но оставался flat `bg-theme-bg/60` + ticker solid blue, карта отсутствовала, контакты дублировались, нет back-to-top, useful links без logos grid, нет glass architecture.
- AFTER: `src/components/Footer.tsx` multi-layer (deep navy #050f1e + gradient/circuit 0.04/0.07 + premium glass 30px) — единый GlassSurface family (`--glass-surface` etc.). Useful Links 7 (single source `USEFUL_LINKS`) → GlassLogoCard `160×84` `object-fit contain` `hover gold` `focus ring` carousel/grid с arrow + swipe + keyboard. Court nav 4 + directory 4 региона/77 судов single source `REGIONAL_CLUSTERS` accordion + mini SVG Tajikistan map gold/cyan selected/hover + alt list. Contacts ТАМОС `info@sud.tj` `+992 372331415` `734018 Душанбе` single source `t(contacts.*)` mailto/tel. Bottom bar `rgba 5,15,30,0.65` blur premium + IT badge compact + FloatingGlassButton back-to-top (fixed, smooth, 600px threshold, aria-label, no black square).
- Verify: tsc 0, build 6.35s OK, health/vite/admin 200, footer semantic `footer/nav/section/address/ul/li/a`, @supports fallback, responsive 4→2→stack, a11y WCAG AA, perf lazy logos? n/a (text cards), link validation external `rel noreferrer` crawlable.
...
## GLOBAL_LIQUID_GLASS_PREMIUM_ULTRA 2026-09-24 - BEFORE/AFTER → v2.5.0
- BEFORE (audit): ~190 card/panel surfaces in 3 visual languages: (1) theme-token translucent + blur (portal dark OK, light washed), (2) near-opaque white light branches (`bg-white/95` hubs, `bg-white/75` blueprint nodes, `bg-[#f8fafc]/80` containers, `bg-slate-100/80` + `bg-amber-50/80` selected states), (3) fully opaque panels without blur (`bg-theme-surface` section cards, `bg-theme-bg/bg-theme-bgSec` modal shells, StateDutyCalculator `bg-white`, CourtQrCode `bg-white` tile). Blur values scattered (6/8/10/12/14/20/24px), radii 6–30px, 4+ shadow systems, surface logic copied per component.
- AFTER: single token family in tokens.css + `.glass*` primitives in index.css; `.content-card` on glass tokens; ~55 components migrated (kit, shell, modals, e-court, news, sections, court network, pages); white boxes eliminated; blueprint/judicial-map dark glow subsystems preserved (light branches tokenized); documented exceptions: document paper, admin dark kit, inputs/tables/micro-rows, tooltips/badges, a11y opaque mode, fullscreen takeover shells.
- Verification 2026-09-24: tsc 0 errors; build:client ~6.6s OK; smoke 8/8; routes 9/9 200 (incl. 404); SSR DOM: / 51 glass + 0 opaque-white, /courts/dushanbe 8 + 0, /sitemap 10 + 0. No logic/content/route/API changes (class strings only, +3 dead `isDark`/`useTheme` removals). Screenshots (desktop/tablet/mobile) + visual sign-off: user in Chrome (sandbox has no browser tooling).
