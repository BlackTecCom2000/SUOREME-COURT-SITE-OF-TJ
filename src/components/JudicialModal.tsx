...
import { NEWS_REGIONS } from '../data/portalLinks';
import { hasCourtSite } from '../sites/registry';
...
export const JudicialModal: React.FC<JudicialModalProps> = ({
  ...
  const [questionnaireForm, setQuestionnaireForm] = useState({ name: '', phone: '', topic: '', rating: 5, message: '' });
  const [newsRegion, setNewsRegion] = useState('all');
  const [apiCourts, setApiCourts] = useState<any[]>([]);
  const [apiHearings, setApiHearings] = useState<any[]>([]);

  const hostOf = (url?: string | null) => {
    try { return url ? new URL(url).hostname : ''; } catch { return ''; }
  };
  const fmtDbDate = (iso?: string | null) => {
    const m = /^(\d{4})-(\d{2})-(\d{2})/.exec(iso || '');
    return m ? `${m[3]}.${m[2]}.${m[1]}` : (iso || '');
  };
  // Live API rows mapped to the static shapes (admin edits appear instantly)
  const liveCourts = apiCourts.map((c: any) => ({
    id: 'db' + c.id,
    domain: hostOf(c.website),
    url: c.website,
    regionId: c.region,
    nameRu: c.name_ru, nameTj: c.name_tj, nameEn: c.name_en,
    shortNameRu: c.short_name_ru || c.name_ru, shortNameTj: c.short_name_tj || c.name_tj, shortNameEn: c.short_name_en || c.name_en,
    type: c.type, addressRu: c.address_ru, phone: c.phone, email: c.email, status: 'online',
  }));
  const liveHearings = apiHearings.map((h: any) => ({
    id: 'h' + h.id,
    courtRu: h.court_ru, courtTj: h.court_tj, courtEn: h.court_en,
    judgeRu: h.judge_ru || '', judgeTj: h.judge_tj || '',
    date: fmtDbDate(h.hearing_date), time: h.hearing_time || '',
    categoryRu: h.category_ru, categoryTj: h.category_tj, categoryEn: h.category_en,
    partiesRu: h.parties_ru, partiesTj: h.parties_tj, partiesEn: h.parties_en,
    room: h.room || '',
  }));
  const courtsSource = (liveCourts.length > 0 ? liveCourts : REGIONAL_COURTS) as any[];
  const hearingsSource = (liveHearings.length > 0 ? liveHearings : HEARINGS_SCHEDULE) as any[];
  ...
  useEffect(() => {
    if (isOpen && activeTab === 'news') {
      setLoadingNews(true);
      fetch(`/api/news?region=${newsRegion}`).then(r => r.json()).then(d => { setNews(d); setLoadingNews(false); }).catch(() => setLoadingNews(false));
    }
    ...
    if (isOpen && activeTab === 'announcements' && announcements.length === 0) {
      setLoadingExtra(true);
      fetch('/api/announcements').then(r => r.json()).then(d => { setAnnouncements(d); setLoadingExtra(false); }).catch(() => setLoadingExtra(false));
    }
    if (isOpen && activeTab === 'vacancies' && vacancies.length === 0) {
      setLoadingExtra(true);
      fetch('/api/vacancies').then(r => r.json()).then(d => { setVacancies(d); setLoadingExtra(false); }).catch(() => setLoadingExtra(false));
    }
    if (isOpen && activeTab === 'journal' && journal.length === 0) {
      setLoadingExtra(true);
      fetch('/api/journal').then(r => r.json()).then(d => { setJournal(d); setLoadingExtra(false); }).catch(() => setLoadingExtra(false));
    }
    if (isOpen && activeTab === 'stats' && !stats) {
      setLoadingExtra(true);
      fetch('/api/stats').then(r => r.json()).then(d => { setStats(d); setLoadingExtra(false); }).catch(() => setLoadingExtra(false));
    }
    if (isOpen && activeTab === 'courts' && apiCourts.length === 0) {
      fetch('/api/courts').then(r => r.json()).then(d => { if (Array.isArray(d) && d.length > 0) setApiCourts(d); }).catch(() => undefined);
    }
    if (isOpen && activeTab === 'hearings' && apiHearings.length === 0) {
      fetch('/api/hearings').then(r => r.json()).then(d => { if (Array.isArray(d) && d.length > 0) setApiHearings(d); }).catch(() => undefined);
    }
  }, [isOpen, activeTab, newsRegion, acts.length, announcements.length, vacancies.length, journal.length, stats]);

  const pickTitle = (item: any) => language === 'en' ? (item.title_en || item.title_ru) : language === 'tj' ? (item.title_tj || item.title_ru) : item.title_ru;
  const pickExcerpt = (item: any) => language === 'en' ? (item.excerpt_en || item.excerpt_ru) : language === 'tj' ? (item.excerpt_tj || item.excerpt_ru) : item.excerpt_ru;

  const submitQuestionnaire = () => {
    if (questionnaireForm.name.trim().length < 2) return;
    fetch('/api/questionnaire', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(questionnaireForm),
    }).then(r => { if (r.ok) setQuestionnaireSent(true); }).catch(() => undefined);
  };
  ...
  const filteredCourts = courtsSource.filter((court) => {
  ...
  // Filter Hearings
  const filteredHearings = hearingsSource.filter((h) => {
      ...
      >
        ...
        <div className="flex items-center gap-1 sm:gap-1.5 px-6 py-2 border-b border-theme-border overflow-x-auto bg-theme-bg text-xs font-mono scrollbar-none">
          {[
            ...
            { id: 'announcements', labelRu: 'Объявления', labelTj: 'Эълонҳо', labelEn: 'Announcements' },
            { id: 'vacancies', labelRu: 'Вакансии', labelTj: 'Ҷойҳои корӣ', labelEn: 'Vacancies' },
            { id: 'stats', labelRu: 'Статистика', labelTj: 'Омор', labelEn: 'Statistics' },
            { id: 'journal', labelRu: 'Журнал', labelTj: 'Нашрия', labelEn: 'Journal' },
            { id: 'questionnaire', labelRu: 'Анкета', labelTj: 'Саволнома', labelEn: 'Survey' },
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 text-sm text-theme-textSec font-sans leading-relaxed scrollbar-thin">
                  ...
                  >
                    ...
                    <div className="mt-3 pt-2 border-t border-theme-border font-mono text-[10px] text-theme-textMuted group-hover:text-theme-gold flex items-center justify-between gap-2">
                      ...
                      {hasCourtSite(court.domain.split('.')[0]) && (
                        <a
                          href={'/courts/' + court.domain.split('.')[0]}
                          onClick={(e) => e.stopPropagation()}
                          className="inline-flex items-center gap-1 px-2 py-1 rounded-lg border border-theme-gold/50 text-theme-gold hover:bg-theme-gold hover:text-black transition-colors"
                        >
            ...
            <div className="space-y-5">
              ...
              <div className="flex items-center gap-1 overflow-x-auto text-[11px] font-mono scrollbar-none">
                {NEWS_REGIONS.map((r) => (
                  ...
                  >
                    {language === 'en' ? r.labelEn : language === 'tj' ? r.labelTj : r.labelRu}
                  </button>
                ))}
              </div>
                ...
                {loadingNews ? (
                  ...
                  >
                    ...
                    <div className="mt-3 pt-2 border-t border-theme-border flex items-center justify-end">
                      <a
                        href={`/news/${item.slug}`}
                        className="font-mono text-xs px-3 py-1.5 rounded-lg border border-theme-border hover:bg-theme-gold hover:text-black transition-colors inline-flex items-center gap-1.5"
                      >
          ...
          {activeTab === 'announcements' && (
            <div className="space-y-5">
              <div className="border-l-2 border-theme-gold pl-4 py-1">
                <span className="font-mono text-xs text-theme-gold tracking-widest uppercase">
                  {language === 'en' ? 'Official Notices' : language === 'tj' ? 'Эълонҳои расмӣ' : 'Официальные объявления'}
                </span>
                <h3 className="text-xl font-medium text-theme-text tracking-tight mt-1">
                  {language === 'en' ? 'Announcements of the Supreme Court' : language === 'tj' ? 'Эълонҳои Суди Олӣ' : 'Объявления Верховного суда'}
                </h3>
              </div>
              ...
              <div className="space-y-3 pt-2">
                {loadingExtra ? (
                  ...
                  <div className="text-center py-4 text-theme-textSec font-mono text-xs">No announcements found</div>
                  ...
                  >
                    ...
                    <h4 className="text-sm font-medium text-theme-text mb-1.5 leading-snug">
                      {pickTitle(item)}
                    </h4>
                    {pickExcerpt(item) && pickExcerpt(item) !== pickTitle(item) && (
                      <p className="text-xs text-theme-textSec leading-relaxed">{pickExcerpt(item)}</p>
                    )}
                    <div className="mt-3 pt-2 border-t border-theme-border flex items-center justify-end">
                      <a
          ...
          {activeTab === 'vacancies' && (
            <div className="space-y-5">
              <div className="border-l-2 border-theme-gold pl-4 py-1">
                <span className="font-mono text-xs text-theme-gold tracking-widest uppercase">
                  {language === 'en' ? 'Civil Service Competition' : language === 'tj' ? 'Озмуни хизмати давлатӣ' : 'Конкурс на госслужбу'}
                </span>
                <h3 className="text-xl font-medium text-theme-text tracking-tight mt-1">
                  {language === 'en' ? 'Vacant Positions' : language === 'tj' ? 'Ҷойҳои кори холӣ' : 'Вакантные должности'}
                </h3>
              </div>
              ...
              <div className="space-y-3 pt-2">
                {loadingExtra ? (
                  ...
                  <div className="text-center py-4 text-theme-textSec font-mono text-xs">No vacancies found</div>
                ) : vacancies.map((item) => (
                  <div
                    key={item.id}
                    className="p-4 rounded-xl bg-theme-surface border border-theme-border hover:border-theme-gold transition-all shadow-xs"
          ...
          {activeTab === 'stats' && (
            <div className="space-y-5">
              <div className="border-l-2 border-theme-gold pl-4 py-1">
                <span className="font-mono text-xs text-theme-gold tracking-widest uppercase">
                  {language === 'en' ? 'Judicial Statistics' : language === 'tj' ? 'Омори судӣ' : 'Судебная статистика'}
                </span>
                <h3 className="text-xl font-medium text-theme-text tracking-tight mt-1">
                  {language === 'en' ? 'Courts in Numbers' : language === 'tj' ? 'Суд дар рақамҳо' : 'Суды в цифрах'}
                </h3>
              </div>
              ...
              {loadingExtra || !stats ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-2">
                  {[
                    { value: stats.courts, ru: 'Судов', tj: 'Судҳо', en: 'Courts' },
                    { value: stats.hearings, ru: 'Заседаний', tj: 'Маҷлисҳо', en: 'Hearings' },
                    { value: stats.acts, ru: 'Актов', tj: 'Санадаҳо', en: 'Acts' },
                    { value: stats.news, ru: 'Новостей', tj: 'Хабарҳо', en: 'News' },
                    { value: stats.announcements, ru: 'Объявлений', tj: 'Эълонҳо', en: 'Notices' },
                    { value: stats.vacancies, ru: 'Вакансий', tj: 'Ҷойҳои корӣ', en: 'Vacancies' },
                  ].map((s) => (
                    <div
                      key={s.en}
                    ...
                    >
                      ...
                      <div className="font-mono text-2xl font-bold text-theme-text"><SlidingNumber value={s.value} /></div>
                      <div className="font-mono text-[11px] text-theme-textSec uppercase tracking-wider mt-1">
                        {language === 'en' ? s.en : language === 'tj' ? s.tj : s.ru}
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {activeTab === 'journal' && (
            <div className="space-y-5">
              <div className="border-l-2 border-theme-gold pl-4 py-1">
                <span className="font-mono text-xs text-theme-gold tracking-widest uppercase">
                  {language === 'en' ? 'Official Periodical' : language === 'tj' ? 'Нашрияи расмӣ' : 'Официальное издание'}
                </span>
                <h3 className="text-xl font-medium text-theme-text tracking-tight mt-1">
                  {language === 'en' ? 'Supreme Court Journal' : language === 'tj' ? 'Нашрияи Суди Олӣ' : 'Журнал Верховного суда'}
                </h3>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                {loadingExtra ? (
                  ...
                  <div className="text-center py-4 text-theme-textSec font-mono text-xs col-span-2">No issues found</div>
                ) : journal.map((item) => (
                  <div
                    key={item.id}
                    className="p-4 rounded-xl bg-theme-surface border border-theme-border hover:border-theme-gold transition-all flex flex-col justify-between shadow-xs"
          ...
          {activeTab === 'questionnaire' && (
            <div className="space-y-5">
              <div className="border-l-2 border-theme-gold pl-4 py-1">
                <span className="font-mono text-xs text-theme-gold tracking-widest uppercase">
                  {language === 'en' ? 'Your Opinion Matters' : language === 'tj' ? 'Андешаи шумо муҳим аст' : 'Ваше мнение важно'}
                </span>
                <h3 className="text-xl font-medium text-theme-text tracking-tight mt-1">
                  {language === 'en' ? 'Citizen Survey' : language === 'tj' ? 'Саволномаи шаҳрвандон' : 'Анкета граждан'}
                </h3>
              </div>

              {questionnaireSent ? (
                <div className="p-6 rounded-xl bg-theme-surface border border-emerald-400/40 text-center">
                  <CheckCircle2 size={28} className="text-emerald-400 mx-auto mb-2" />
                  <div className="text-sm font-medium text-theme-text">
                    {language === 'en' ? 'Thank you! Your response has been received.' : language === 'tj' ? 'Ташаккур! Ҷавоби шумо қабул шуд.' : 'Спасибо! Ваш ответ получен.'}
                  </div>
                </div>
              ) : (
                <div className="p-5 rounded-xl bg-theme-surface border border-theme-border space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <input
                      ...
                      placeholder={language === 'en' ? 'Full name' : language === 'tj' ? 'Ному насаб' : 'ФИО'}
                      className="w-full bg-theme-bg border border-theme-border rounded-xl px-4 py-2.5 text-xs text-theme-text placeholder-theme-textMuted focus:outline-none focus:border-theme-gold font-mono transition-colors"
                    />
                    <input
                      type="text"
                      value={questionnaireForm.phone}
                      onChange={(e) => setQuestionnaireForm({ ...questionnaireForm, phone: e.target.value })}
                      placeholder={language === 'en' ? 'Phone (optional)' : language === 'tj' ? 'Телефон (ихтиёрӣ)' : 'Телефон (необязательно)'}
                      className="w-full bg-theme-bg border border-theme-border rounded-xl px-4 py-2.5 text-xs text-theme-text placeholder-theme-textMuted focus:outline-none focus:border-theme-gold font-mono transition-colors"
                    />
                  </div>
                  <input
                    type="text"
                    value={questionnaireForm.topic}
                    onChange={(e) => setQuestionnaireForm({ ...questionnaireForm, topic: e.target.value })}
                    placeholder={language === 'en' ? 'Topic (optional)' : language === 'tj' ? 'Мавзӯъ (ихтиёрӣ)' : 'Тема (необязательно)'}
                    className="w-full bg-theme-bg border border-theme-border rounded-xl px-4 py-2.5 text-xs text-theme-text placeholder-theme-textMuted focus:outline-none focus:border-theme-gold font-mono transition-colors"
                  />
                  <div>
                    <div className="font-mono text-[11px] text-theme-textSec uppercase tracking-wider mb-2">
                      {language === 'en' ? 'Rate court services (1-5)' : language === 'tj' ? 'Хизматрасонии судҳоро баҳо диҳед (1-5)' : 'Оцените работу судов (1-5)'}
                    </div>
                    <div className="flex items-center gap-1.5">
                      {[1, 2, 3, 4, 5].map((r) => (
                        <button
                          key={r}
                          type="button"
                          onClick={() => setQuestionnaireForm({ ...questionnaireForm, rating: r })}
                          aria-label={`rating ${r}`}
                          className={`p-1.5 rounded-lg border transition-colors ${
                            questionnaireForm.rating >= r
                              ? 'border-theme-gold text-theme-gold'
                              : 'border-theme-border text-theme-textMuted'
                          }`}
                        >
                          <Star size={16} fill={questionnaireForm.rating >= r ? 'currentColor' : 'none'} />
                        </button>
                      ))}
                    </div>
                  </div>
                  <textarea
                    value={questionnaireForm.message}
                    onChange={(e) => setQuestionnaireForm({ ...questionnaireForm, message: e.target.value })}
                    placeholder={language === 'en' ? 'Your message (optional)' : language === 'tj' ? 'Паёми шумо (ихтиёрӣ)' : 'Ваше сообщение (необязательно)'}
                    rows={4}
                    className="w-full bg-theme-bg border border-theme-border rounded-xl px-4 py-2.5 text-xs text-theme-text placeholder-theme-textMuted focus:outline-none focus:border-theme-gold font-sans transition-colors resize-none"
                  />
                  <button
                    type="button"
                    onClick={submitQuestionnaire}
                    disabled={questionnaireForm.name.trim().length < 2}
                    className="inline-flex items-center gap-2 rounded-xl border border-theme-gold bg-theme-gold/15 px-5 py-2.5 font-mono text-xs uppercase tracking-wider text-theme-text transition-all hover:bg-theme-gold hover:text-black disabled:opacity-40 disabled:pointer-events-none"
                  >
                    <Send size={13} />
                    <span>{language === 'en' ? 'Submit' : language === 'tj' ? 'Фиристодан' : 'Отправить'}</span>
                  </button>
                </div>
              )}
            </div>
