...
// Excerpts are plain-text previews: strip any editor HTML so that
// untrusted markup can never execute in the public page (stored XSS).
                ...
                {news.slice(0, 3).map((item, idx) => (
                  <Reveal key={item.id} delay={250 + idx * 60}>
                    ...
                    >
                      ...
                      <p className="text-[11px] text-theme-textSec line-clamp-2 leading-relaxed">{stripHtml((language === 'en' ? (item.body_en || item.body_ru) : language === 'tj' ? (item.body_tj || item.body_ru) : item.body_ru) || '')}</p>
                    </div>
                  </Reveal>
                ))}
              </div>
